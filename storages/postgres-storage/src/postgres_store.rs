...
impl PostgresStore {
    ...
    pub async fn load_device_data_for_device(&self, device_id: i32) -> Result<Option<CoreDevice>> {
        let row = sqlx::query(
            "SELECT id, lid, pn, registration_id, noise_key, identity_key, signed_pre_key,
                    ...
                    nct_salt, server_has_prekeys, server_cert_chain, login_counter
        ...
        let pn = if !pn_str.is_empty() { pn_str.parse().ok() } else { None };
        ...
        let adv_secret_key: [u8; 32] = adv_bytes.try_into().map_err(|_| {
            StoreError::Validation("invalid adv_secret_key length".into())
        })?;
    ...
    pub async fn create_new_device(&self) -> Result<i32> {
        ...
        self.save_device_data_for_device(self.device_id, &new_device).await?;
        Ok(self.device_id)
    }
...
impl SignalStore for PostgresStore {
    async fn put_identity(&self, address: &str, key: [u8; 32]) -> Result<()> {
        self.put_identity_for_device(address, key, self.device_id).await
    }

    async fn load_identity(&self, address: &str) -> Result<Option<[u8; 32]>> {
        let blob = self.load_identity_for_device(address, self.device_id).await?;
        match blob {
            None => Ok(None),
    ...
    async fn delete_identity(&self, address: &str) -> Result<()> {
        self.delete_identity_for_device(address, self.device_id).await
    }
    ...
    async fn put_session(&self, address: &str, session: &[u8]) -> Result<()> {
        self.put_session_for_device(address, session, self.device_id).await
    }

    async fn delete_session(&self, address: &str) -> Result<()> {
        self.delete_session_for_device(address, self.device_id).await
    }
    ...
    async fn load_prekeys_batch(&self, ids: &[u32]) -> Result<Vec<(u32, Bytes)>> {
        ...
        Ok(rows
            .into_iter()
            .map(|r| (r.get::<i32, _>("id") as u32, Bytes::from(r.get::<Vec<u8>, _>("key"))))
            .collect())
    }
    ...
    async fn load_signed_prekey(&self, id: u32) -> Result<Option<Vec<u8>>> {
        let row =
            sqlx::query("SELECT record FROM signed_prekeys WHERE id = $1 AND device_id = $2")
                .bind(id as i32)
                .bind(self.device_id)
                .fetch_optional(&self.pool)
                .await
    ...
    async fn put_sender_key(&self, address: &str, record: &[u8]) -> Result<()> {
        self.put_sender_key_for_device(address, record, self.device_id).await
    }

    async fn get_sender_key(&self, address: &str) -> Result<Option<Vec<u8>>> {
        self.get_sender_key_for_device(address, self.device_id).await
    }

    async fn delete_sender_key(&self, address: &str) -> Result<()> {
        self.delete_sender_key_for_device(address, self.device_id).await
    }
}
...
impl AppSyncStore for PostgresStore {
    async fn get_sync_key(&self, key_id: &[u8]) -> Result<Option<AppStateSyncKey>> {
        self.get_app_state_sync_key_for_device(key_id, self.device_id).await
    }

    async fn set_sync_key(&self, key_id: &[u8], key: AppStateSyncKey) -> Result<()> {
        self.set_app_state_sync_key_for_device(key_id, key, self.device_id).await
    }

    async fn get_version(&self, name: &str) -> Result<HashState> {
        self.get_app_state_version_for_device(name, self.device_id).await
    }

    async fn set_version(&self, name: &str, state: HashState) -> Result<()> {
        self.set_app_state_version_for_device(name, state, self.device_id).await
    }
    ...
    async fn get_mutation_mac(&self, name: &str, index_mac: &[u8]) -> Result<Option<Vec<u8>>> {
        self.get_app_state_mutation_mac_for_device(name, index_mac, self.device_id).await
    }

    async fn delete_mutation_macs(&self, name: &str, index_macs: &[Vec<u8>]) -> Result<()> {
        self.delete_app_state_mutation_macs_for_device(name, index_macs, self.device_id).await
    }

    async fn get_latest_sync_key_id(&self) -> Result<Option<Vec<u8>>> {
        self.get_latest_app_state_sync_key_id_for_device(self.device_id).await
    }
}
...
impl DeviceStore for PostgresStore {
    async fn save(&self, device: &CoreDevice) -> Result<()> {
        self.save_device_data_for_device(self.device_id, device).await
    }
...
mod tests {
    ...
    async fn test_device_create_and_load() {
        ...
        assert!(store.device_exists(store.device_id()).await.unwrap());
        let loaded = store.load_device_data_for_device(store.device_id()).await.unwrap();
    ...
    async fn test_server_cert_chain_roundtrip() {
        ...
        let chain = CachedServerCertChain {
            intermediate: CachedNoiseCert { key: [0xAB; 32], not_before: 1_700_000_000, not_after: 1_900_000_000 },
            leaf: CachedNoiseCert { key: [0xCD; 32], not_before: 1_700_000_500, not_after: 1_899_999_500 },
        };
        ...
        store.save_device_data_for_device(store.device_id(), &device).await.unwrap();
    ...
    async fn test_identity_put_load_delete() {
        let store = create_test_store().await;
        let key = [0xAAu8; 32];
        store.put_identity("addr1@s.whatsapp.net", key).await.unwrap();
        ...
        assert!(store.load_identity("addr1@s.whatsapp.net").await.unwrap().is_none());
    }

    #[tokio::test]
    async fn test_session_put_get_delete() {
        let store = create_test_store().await;
        store.put_session("peer@s.whatsapp.net", &[1, 2, 3, 4]).await.unwrap();
        assert!(store.has_session("peer@s.whatsapp.net").await.unwrap());
        let record = store.get_session("peer@s.whatsapp.net").await.unwrap().unwrap();
    ...
    async fn test_sender_key_put_get_delete() {
        let store = create_test_store().await;
        store.put_sender_key("group1@g.us:0@lid", &[0xDD; 64]).await.unwrap();
        let loaded = store.get_sender_key("group1@g.us:0@lid").await.unwrap().unwrap();
        assert_eq!(loaded, vec![0xDD; 64]);
        store.delete_sender_key("group1@g.us:0@lid").await.unwrap();
        assert!(store.get_sender_key("group1@g.us:0@lid").await.unwrap().is_none());
    }
    ...
    async fn test_device_registry_save_and_get() {
        ...
        let record = DeviceListRecord {
            ...
            devices: vec![
                ...
                DeviceInfo { device_id: 1, key_index: Some(42) },
            ],
    ...
    async fn test_device_registry_update_existing() {
        ...
        let record1 = DeviceListRecord {
            user: "9876543210".to_string(),
            devices: vec![DeviceInfo { device_id: 0, key_index: None }],
            timestamp: 1000,
        ...
        let record2 = DeviceListRecord {
            ...
            devices: vec![
                DeviceInfo { device_id: 0, key_index: None },
                DeviceInfo { device_id: 2, key_index: None },
            ],
            timestamp: 2000,
    ...
    async fn test_device_registry_get_nonexistent() {
        let store = create_test_store().await;
        assert!(store.get_devices("nonexistent_user").await.unwrap().is_none());
    }
    ...
    async fn test_sender_key_devices_upsert_overwrites() {
        ...
        store.set_sender_key_status(group, &[("user1:5@lid", false)]).await.unwrap();
        store.set_sender_key_status(group, &[("user1:5@lid", true)]).await.unwrap();
        let devices = store.get_sender_key_devices(group).await.unwrap();
    ...
    async fn test_tc_token_upsert() {
        let store = create_test_store().await;
        let e1 = TcTokenEntry { token: vec![1, 2, 3], token_timestamp: 1000, sender_timestamp: None };
        store.put_tc_token("user2@lid", &e1).await.unwrap();
        let e2 = TcTokenEntry { token: vec![4, 5, 6], token_timestamp: 2000, sender_timestamp: Some(1500) };
    ...
    async fn test_tc_token_get_all_jids() {
        let store = create_test_store().await;
        let entry = TcTokenEntry { token: vec![1], token_timestamp: 1000, sender_timestamp: None };
    ...
    async fn test_tc_token_delete_expired() {
        let store = create_test_store().await;
        let old = TcTokenEntry { token: vec![1], token_timestamp: 1000, sender_timestamp: None };
        let recent = TcTokenEntry { token: vec![2], token_timestamp: 5000, sender_timestamp: None };
    ...
    async fn test_sent_message_store_take() {
        ...
        store
            ...
            .await
            .unwrap();
        let payload = store.take_sent_message("chat@s.whatsapp.net", "msg001").await.unwrap();
        assert_eq!(payload, Some(vec![0xDE, 0xAD]));
        // second take returns None (consumed)
        let again = store.take_sent_message("chat@s.whatsapp.net", "msg001").await.unwrap();
    ...
    async fn test_sent_message_delete_expired() {
        ...
        store.store_sent_message("chat@s.whatsapp.net", "old_msg", &[1]).await.unwrap();
        store.store_sent_message("chat@s.whatsapp.net", "new_msg", &[2]).await.unwrap();
        ...
        assert_eq!(deleted, 1);
        assert!(store.take_sent_message("chat@s.whatsapp.net", "old_msg").await.unwrap().is_none());
        assert!(store.take_sent_message("chat@s.whatsapp.net", "new_msg").await.unwrap().is_some());
    }
    ...
    async fn test_base_key_save_check_delete() {
        ...
        store.save_base_key("peer@s.whatsapp.net", "msgXYZ", &base_key).await.unwrap();
        assert!(store.has_same_base_key("peer@s.whatsapp.net", "msgXYZ", &base_key).await.unwrap());
        assert!(!store.has_same_base_key("peer@s.whatsapp.net", "msgXYZ", &[0xFF; 32]).await.unwrap());
        store.delete_base_key("peer@s.whatsapp.net", "msgXYZ").await.unwrap();
        assert!(!store.has_same_base_key("peer@s.whatsapp.net", "msgXYZ", &base_key).await.unwrap());
    }
    ...
    async fn test_app_state_sync_key_upsert() {
        ...
        store
            .set_sync_key(&key_id, AppStateSyncKey { key_data: vec![1; 32], fingerprint: vec![], timestamp: 1 })
            .await
            .unwrap();
        store
            .set_sync_key(&key_id, AppStateSyncKey { key_data: vec![2; 32], fingerprint: vec![], timestamp: 2 })
            .await
            .unwrap();
        let loaded = store.get_sync_key(&key_id).await.unwrap().unwrap();
    ...
    async fn test_app_state_version_roundtrip() {
        ...
        state.version = 42;
        state.hash = [0xAB; 128];
        store.set_version("critical_unblock_to_sync", state.clone()).await.unwrap();
        ...
        let other = store.get_version("regular_high_level_contact_node").await.unwrap();
    ...
    async fn test_app_state_mutation_macs_put_get_delete() {
        ...
        let m1 = AppStateMutationMAC { index_mac: vec![0x01; 32], value_mac: vec![0xAA; 32] };
        let m2 = AppStateMutationMAC { index_mac: vec![0x02; 32], value_mac: vec![0xBB; 32] };

        store.put_mutation_macs(name, 1, &[m1.clone(), m2.clone()]).await.unwrap();

        let got = store.get_mutation_mac(name, &[0x01; 32]).await.unwrap().unwrap();
        assert_eq!(got, vec![0xAA; 32]);
        let got2 = store.get_mutation_mac(name, &[0x02; 32]).await.unwrap().unwrap();
        assert_eq!(got2, vec![0xBB; 32]);

        store.delete_mutation_macs(name, &[vec![0x01; 32]]).await.unwrap();
        assert!(store.get_mutation_mac(name, &[0x01; 32]).await.unwrap().is_none());
        // m2 unaffected
        assert!(store.get_mutation_mac(name, &[0x02; 32]).await.unwrap().is_some());
    }
    ...
    async fn test_app_state_mutation_macs_upsert() {
        ...
        let m = AppStateMutationMAC { index_mac: vec![0x11; 32], value_mac: vec![0x22; 32] };
        store.put_mutation_macs(name, 1, &[m]).await.unwrap();

        let m_updated = AppStateMutationMAC { index_mac: vec![0x11; 32], value_mac: vec![0x33; 32] };
        store.put_mutation_macs(name, 2, &[m_updated]).await.unwrap();

        let got = store.get_mutation_mac(name, &[0x11; 32]).await.unwrap().unwrap();
    ...
    async fn test_update_device_lists_batch() {
        ...
        let records = vec![
            ...
            DeviceListRecord {
                user: "batch_user2".to_string(),
                devices: vec![DeviceInfo { device_id: 0, key_index: Some(1) }],
                timestamp: 200,
                phash: Some("2:xyz".to_string()),
                raw_id: Some(7),
            },
        ];
        ...
        assert_eq!(r2.phash, Some("2:xyz".to_string()));
    ...
    async fn test_sender_key_devices_different_groups() {
        ...
        store.set_sender_key_status(group1, &[("user:5@lid", true)]).await.unwrap();
    ...
    async fn test_clear_all_sender_key_devices() {
        let store = create_test_store().await;
        store.set_sender_key_status("groupX@g.us", &[("u1:0@lid", true)]).await.unwrap();
        store.set_sender_key_status("groupY@g.us", &[("u2:0@lid", true)]).await.unwrap();
        store.clear_all_sender_key_devices().await.unwrap();
        assert!(store.get_sender_key_devices("groupX@g.us").await.unwrap().is_empty());
        assert!(store.get_sender_key_devices("groupY@g.us").await.unwrap().is_empty());
    }
    ...
    async fn test_delete_sender_key_device_rows() {
        ...
        store
            .set_sender_key_status(group, &[("u1:0@lid", true), ("u2:0@lid", false), ("u3:0@lid", true)])
            .await
            .unwrap();
        store.delete_sender_key_device_rows(&["u1:0@lid", "u3:0@lid"]).await.unwrap();
    ...
    async fn test_load_device_data_nonexistent_returns_none() {
        ...
        let result = store.load_device_data_for_device(store.device_id()).await.unwrap();
        assert!(result.is_none());
    }
